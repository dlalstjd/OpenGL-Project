...
void create_font_textures()
{
	...
	for (GLubyte c = 0; c < 128; c++)
	{
		...
		glBindTexture(GL_TEXTURE_3D, texture_text);
		glTexImage3D(GL_TEXTURE_3D, 0, GL_R8, width, height, 1, 0, GL_RED, GL_UNSIGNED_BYTE, bitmap);
...
bool init_text()
{
	...
	vertex vertices[6] =
	{
		{ vec3(0,1,0), vec3(0,0,0), vec3(0.0f,0.0f,1.0f) },
		{ vec3(0,0,0), vec3(0,0,0), vec3(0.0f,1.0f,1.0f) },
		{ vec3(1,0,0), vec3(0,0,0), vec3(1.0f,1.0f,1.0f) },
		{ vec3(0,1,0), vec3(0,0,0), vec3(0.0f,0.0f,1.0f) },
		{ vec3(1,0,0), vec3(0,0,0), vec3(1.0f,1.0f,1.0f) },
		{ vec3(1,1,0), vec3(0,0,0), vec3(1.0f,0.0f,1.0f) }
	};
...
void render_text(std::string text, GLint _x, GLint _y, GLfloat scale, vec4 color, GLfloat dpi_scale)
{
	...
	for (c = text.begin(); c != text.end(); c++)
	{
		...
		glBindTexture(GL_TEXTURE_3D, ch.textureID);
